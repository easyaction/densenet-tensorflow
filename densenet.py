...
class DenseNet:
    ...
    def build_networks(self,input_tensor, num_classes,growth_rate=12,is_training=True, name="densenet"):
        ...
        output_t, out_channels = self.dense_block(output_t, l=6, k=growth_rate, keep_prob=0.2, name="denseB_%d" % 1, is_training=is_training)
        output_t = self.transition_layer(output_t, in_channels=out_channels, c_rate=0.5, name="transL_%d" % 1, is_training=is_training)

        output_t, out_channels = self.dense_block(output_t, l=12, k=growth_rate, keep_prob=0.2, name="denseB_%d" % 2, is_training=is_training)
        output_t = self.transition_layer(output_t, in_channels=out_channels, c_rate=0.5, name="transL_%d" % 2, is_training=is_training)

        output_t, out_channels = self.dense_block(output_t, l=24, k=growth_rate, keep_prob=0.2, name="denseB_%d" % 3, is_training=is_training)
        output_t = self.transition_layer(output_t, in_channels=out_channels, c_rate=0.5, name="transL_%d" % 3, is_training=is_training)

        output_t, out_channels = self.dense_block(output_t, l=16, k=growth_rate, keep_prob=0.2, name="denseB_%d" % 4, is_training=is_training)

        logits = self.classification_layer(output_t,num_classes=num_classes,name="classL",keep_prob=0.2,is_training=is_training)
